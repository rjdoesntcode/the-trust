...
The first build failed instantly because no `wrangler.toml` existed; after this change the same settings succeed. Custom domain: Worker → Settings → Domains & Routes → add `trust.forum` (and `www.trust.forum`). Static-asset Workers honor `_headers` and `_redirects` in the asset directory, so the CSP and caching headers apply. Verify after deploy: `curl -sI https://trust.forum/ | grep -i content-security-policy`.
...
`.github/workflows/build.yml` builds on every push and deploys `site/_site` to GitHub Pages from `main`. Enable once in Settings → Pages → Source: GitHub Actions; until then the deploy job prints a warning and passes, so CI stays green and the mirror simply does not exist yet. GitHub Pages cannot send custom headers, so every page carries a `<meta http-equiv="Content-Security-Policy">` and `<meta name="referrer" content="no-referrer">` as the fallback. `.nojekyll` is emitted so `_headers` and other underscore paths are served.
