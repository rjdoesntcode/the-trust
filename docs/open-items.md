...
3. **Bridge: the maintainer states revision 3 is current.** Only revisions 1 and 2 were in the upload. `revisions.json` now expects `the-bridge-rev3.md` (tag `bridge-rev3`) as current; until it is uploaded the site shows revision 2 as the latest published revision and says revision 3 is expected. Upload `the-bridge-rev3.md` (and its drafting record, if one exists) to a branch and I will attribute, hash and publish it.
